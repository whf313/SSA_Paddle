...
def get_postion_ids_vectorized(starts_pos: np.ndarray, block_lengths: np.ndarray) -> np.ndarray:
    """
    【已优化】以向量化的方式生成全局位置ID。
    """
    # 为每个块创建一个 range，然后一次性拼接起来
    return np.concatenate([np.arange(start, start + length) for start, length in zip(starts_pos, block_lengths)])
...
def get_example_ssa(tokenized_source, tokenizer, data_args):
    ...
    len_input = len(ids)
    min_len = min(data_args.block_min_length, len_chunk // num_blocks)

    if len_input < num_blocks:
        raise ValueError("The input sequence is too short to generate N non-overlapping subsequences.")

    # 初始化结果变量
    chunked_ids, labels, lengths, start_pos = [], [], [], []
    total_len = 0
    pre_index = 0
    ...
    for i in range(num_blocks):
        if i == 0:
            # 第一个块：从0开始
            seq_len = random.randint(min_len, (len_chunk - total_len) // (num_blocks - i))
            start_index = 0
            end_index = seq_len
            
        elif i == num_blocks - 1:
            # 最后一个块：确保总长度为len_chunk
            seq_len = len_chunk - total_len
            end_index = random.randint(pre_index + seq_len, len_input - 1)
            start_index = end_index - seq_len
            
        else:
            # 中间块：在合理范围内随机选择
            block_end = (len_input // num_blocks) * (i + 1)
            end_index = random.randint(pre_index + min_len, block_end)
            
            # 计算序列长度的约束
            remaining_blocks = num_blocks - i
            remaining_capacity = len_chunk - total_len
            max_for_last = len_input - end_index - 1  # 为最后一个块预留空间
            
            min_seq_len = max(min_len, remaining_capacity - max_for_last)
            max_seq_len = min(end_index - pre_index, remaining_capacity // remaining_blocks)
            max_seq_len = max(max_seq_len, min_seq_len)  # 确保上界不小于下界
            
            seq_len = random.randint(min_seq_len, max_seq_len)
            start_index = end_index - seq_len

        # 提取子序列和标签
        subseq = ids[start_index:end_index]
        label = ids[start_index + 1:end_index + 1]

        # 更新结果
        chunked_ids.extend(subseq)
        labels.extend(label)
    ...
    # 生成位置ID和稀疏度级别
    pos_ids = get_postion_ids_vectorized(start_pos, lengths)
...
# def get_example_ssa_copy(tokenized_source, tokenizer, data_args):
    
#     ids = tokenized_source["input_ids"]
#     len_chunk = min(len(ids), data_args.max_length)            # 重构后序列的长度(4096)
