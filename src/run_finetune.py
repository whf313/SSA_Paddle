...
from paddlenlp.trl import ModelConfig, SFTConfig, SFTTrainer
# from sftdata_config import DataConfig
from utils.argument import DataConfig
...
from ssa_llama_modeling import replace_llama_with_ssa
...
def manual_pad_sequence(sequences, batch_first=True, padding_value=0):
    """手动实现 pad_sequence 功能"""
    # 找到最大长度
    max_len = max(seq.shape[0] for seq in sequences)
    
    # 创建填充后的张量
    if batch_first:
        padded = paddle.full([len(sequences), max_len], padding_value, dtype=sequences[0].dtype)
        for i, seq in enumerate(sequences):
            padded[i, :len(seq)] = seq
    else:
        padded = paddle.full([max_len, len(sequences)], padding_value, dtype=sequences[0].dtype)
        for i, seq in enumerate(sequences):
...
@dataclass
class DataCollatorForSparse(object):
    """Collate examples for supervised fine-tuning."""

    tokenizer: AutoTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, paddle.Tensor]:
        instances = [instance for instance in instances if "block_lengths" in instance]

        input_ids, labels, position_ids, block_lengths, sparsity_levels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "position_ids", "block_lengths", "sparsity_levels"))
        ...
        block_lengths = [paddle.to_tensor(x) for x in block_lengths]
        sparsity_levels = [paddle.to_tensor(x) for x in sparsity_levels]

        # 使用手动实现的 pad_sequence
        input_ids = manual_pad_sequence(
            input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
        labels = manual_pad_sequence(
            labels, batch_first=True, padding_value=IGNORE_INDEX
        )
        position_ids = manual_pad_sequence(
        ...
        return dict(
            ...
            # attention_mask=input_ids.not_equal(self.tokenizer.pad_token_id),
        )
...
def main():
    ...
    if data_args.use_ssa_convert:
        data_collator_fn = DataCollatorForSparse(tokenizer=tokenizer)
    else:
...
def create_dataset(data_args, training_args):
    ...
    dev_ds = None
    test_ds = None
    if os.path.exists(os.path.join(data_args.dataset_name_or_path, "train.jsonl")) or os.path.exists(
        os.path.join(data_args.dataset_name_or_path, "dev.json")
    ):
        if training_args.do_train:
            train_ds = load_dataset(
                "json",
                data_files=os.path.join(data_args.dataset_name_or_path, "train.jsonl"),
                lazy=data_args.lazy,
            )[0]
        if training_args.do_eval:
            dev_ds = load_dataset(
                "json",
                data_files=os.path.join(data_args.dataset_name_or_path, "dev.jsonl"),
                lazy=data_args.lazy,
            )[0]
        if training_args.do_predict:
