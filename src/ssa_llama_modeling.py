...
from paddlenlp.transformers.utils import caculate_llm_per_token_flops
from paddlenlp.transformers.llama.configuration import (
...
def print_gpu_memory(prefix=""):
    """
    打印当前 GPU 的显存使用情况，功能对等于 PyTorch 的实现。
    :param prefix: 打印信息的前缀字符串。
    """
    if not paddle.is_compiled_with_cuda():
        print(f"{prefix} Paddle 未在 CUDA 环境下编译，无法获取 GPU 显存信息。")
        return

    # paddle.device.cuda.memory_allocated() -> 等同于 torch.cuda.memory_allocated()
    # 返回当前设备上被 PTensor 占用的显存大小（Bytes）。
    allocated_mem_mb = paddle.device.cuda.memory_allocated() / 1024**2

    max_allocated_mem_mb = paddle.device.cuda.max_memory_allocated() / 1024**2

    # paddle.device.cuda.memory_reserved() -> 等同于 torch.cuda.memory_reserved()
    # 返回当前设备上被缓存管理的总显存大小（Bytes）。
    reserved_mem_mb = paddle.device.cuda.memory_reserved() / 1024**2

    max_reserved_mem_mb = paddle.device.cuda.max_memory_reserved() / 1024**2

    print(f"{prefix} GPU Memory - Allocated: {allocated_mem_mb:.2f} MB | "
          f"Reserved: {reserved_mem_mb:.2f} MB\n Max_Allocated: {max_allocated_mem_mb:.2f} MB | "
          f"Max_Reserved: {max_reserved_mem_mb:.2f} MB")
...
class LlamaModel(LlamaPretrainedModel):
    ...
    def _prepare_example_mask(self, block_length, sparsity_levels, block_size, device, dtype):
        ...
        total_length = block_length.sum().item() if isinstance(block_length.sum(), paddle.Tensor) else block_length.sum()
        
        # 创建基础的下三角掩码 - 使用更高效的方法
        attention_mask = paddle.full((total_length, total_length), paddle.finfo(dtype).min,dtype=dtype)
        tril_mask = paddle.tril(paddle.ones([total_length, total_length], dtype=paddle.bool))
        attention_mask.masked_fill_(tril_mask, 0)
        
        # 预计算所有块的起始索引
        start_indices = paddle.concat([paddle.zeros(1, dtype=paddle.int64), block_length[:-1].cumsum(0)])
        ...
        for i in range(num_blocks):
            for j in range(i):
                ...
                block_pairs.append((i, j, sparsity, start_indices[i].item(), start_indices[j].item(),
                                block_length[i].item(), block_length[j].item()))
        
        # 批量处理所有块对
        for i, j, sparsity, start_i, start_j, length_i, length_j in block_pairs:
            # 计算子块的数量和选择逻辑
            num_blocks_row = length_i // block_size
            num_blocks_col = length_j // block_size
            total_blocks = num_blocks_row * num_blocks_col
            
            if total_blocks == 0:
                continue
                
            max_blocks_to_select = max(1, int(total_blocks * sparsity))
            
            # 使用paddle操作替代numpy，减少CPU-GPU数据传输
            block_indices = paddle.randperm(total_blocks)[:max_blocks_to_select]
            
            # 计算行列索引
            row_block_indices = block_indices // num_blocks_col
            col_block_indices = block_indices % num_blocks_col
            
            # 批量更新掩码 - 使用高级索引
            for k in range(len(block_indices)):
                row_start = start_i + row_block_indices[k].item() * block_size
                col_start = start_j + col_block_indices[k].item() * block_size
                attention_mask[row_start:row_start + block_size,          # 选中的block设置为0，非选中的设置为-inf
                               col_start:col_start + block_size] = 0
        
        return attention_mask


    def _prepare_block_attention_mask(self, block_length, max_length, inputs_embeds, sparsity_levels, num_group=1, block_size=150):
        """
        优化后的批量注意力掩码生成函数
        """
        ...
        batch_size = len(block_length)
        max_length = max([sum(i) for i in block_length])
        
        # 预计算常量
        group_dim = self.num_heads // num_group
        num_mask = self.num_heads if num_group != 1 else 1
        
        # 初始化掩码张量
        attention_mask = paddle.full(
            (batch_size, num_mask, max_length, max_length), 
            paddle.finfo(dtype).min, 
            dtype=dtype
        )
        
        # 批量处理每个序列
        for batch_idx, (lengths, sparsity_level) in enumerate(zip(block_length, sparsity_levels)):
            total_length = lengths.sum().item() if isinstance(lengths.sum(), paddle.Tensor) else lengths.sum()
            ...
            if num_group != 1:
                ...
                for group_idx in range(num_group):
                    start_head = group_idx * group_dim
                    end_head = (group_idx + 1) * group_dim
                    attention_mask[batch_idx, start_head:end_head, :total_length, :total_length] = example_mask
            else:
                # 所有头使用相同的掩码
                attention_mask[batch_idx, :, :total_length, :total_length] = example_mask
            ...
            if get_env_device() not in ["npu", "mlu"]:
                if is_casual and alibi is None:
                    ...
                    if block_lengths is not None:
                        max_length = max([i.sum() for i in block_lengths])
                        attention_mask = self._prepare_block_attention_mask(
                            # block_lengths, block_lengths.sum(), inputs_embeds, sparsity_levels
                            block_lengths, max_length, inputs_embeds, sparsity_levels
                        )
                    else:
                        attention_mask = None
            else:
...
class SSALlamaForCausalLM(LlamaPretrainedModel):
...
def replace_llama_with_ssa():
    paddlenlp.transformers.llama.modeling.LlamaForCausalLM = SSALlamaForCausalLM